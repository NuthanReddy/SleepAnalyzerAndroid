...
/**
 * Reference-counted shared sensor listener that pipes motion magnitudes into a singleton [MotionBuffer].
 *
 ...
 * MotionMonitor.start()/stop() is refcounted so the listener is only registered while at least one
 * consumer is active.
 */
object MotionMonitor {
    ...
    /** Increment refcount; first caller registers the listener. Returns true if the sensor is now active. */
    @Synchronized
    fun start(context: Context): Boolean {
        if (refs.getAndIncrement() == 0) {
            ...
            val target = linear ?: sm.getDefaultSensor(Sensor.TYPE_ACCELEROMETER) ?: return false
            val l = Listener(usesLinearAcceleration = linear != null)
            listener = l
            sm.registerListener(l, target, SensorManager.SENSOR_DELAY_NORMAL)
        }
    ...
    private class Listener(private val usesLinearAcceleration: Boolean) : SensorEventListener {
        ...
        override fun onSensorChanged(event: SensorEvent) {
            ...
            buffer.add(MotionSample(System.currentTimeMillis(), mag))
        }
